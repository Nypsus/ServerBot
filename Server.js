...
app.post('/webhook', (req, res) => {
    ...
    console.log("Signal reçu : ", signalType); // Afficher le type du signal dans la console


    // Générer deux TradeID uniques (un pour chaque trade)
    const tradeID1 = generateTradeID();
    const tradeID2 = generateTradeID();
    console.log("TradeID 1 : ", tradeID1);  // Afficher les TradeIDs générés
    console.log("TradeID 2 : ", tradeID2);
    ...
    console.log("Message envoyé à Discord : ", discordMessage); // Vérifier le message avant l'envoi


    // Envoyer l'alerte à Discord via le webhook
    axios.post(discordWebhookURL, discordMessage)
        .then(response => {
            // Affichage dans la réponse HTTP au client (par exemple, Postman ou navigateur)
            const successMessage = `
                ✔️ Signal traité avec succès :
                Type: ${signalType}
                TradeID1: ${tradeID1}
                TradeID2: ${tradeID2}
                Le message a été envoyé à Discord.
            `;
            console.log('Message envoyé à Discord');
            res.status(200).send(successMessage);  // Renvoie une réponse détaillée
        })
        .catch(error => {
